  ...
  return (
    <div>
              ...
              <div className="flex">
                <div className="flex flex-col mr-1">
                  {weekDays.map((d) => (
                    <div
                      key={d}
                      className="w-8 h-5 flex items-center justify-center text-xs font-semibold mb-1"
                    >
                      {d}
                    </div>
                  ))}
                  ...
                  {weeks.map((week, i) => (
                    <div key={i} className="flex flex-col gap-1">
                      {week.map((day, j) => (
                        <div
                          key={j}
                          className={`w-5 h-5 rounded border ${day ? getColorClass(usageMap[formatDateSpain(day)]) : "bg-transparent"} border-gray-100`}
                          title={
                            day
      ...
      <div className="flex flex-wrap gap-4 mt-2 border rounded-xl shadow-md bg-white p-5">
        {[
          {
            label: t("Server.details.usageHeatmapNone"),
            count: 0,
          },
          { label: "1", count: 1 },
          { label: "2", count: 2 },
          { label: "3", count: 3 },
          {
            label: t("Server.details.usageHeatmapMore"),
            count: 4,
          },
        ].map(({ label, count }) => (
          <div key={count} className="flex items-center gap-2">
            <span
              className={`inline-block w-4 h-4 rounded border ${getColorClass(count)}`}
            />
